...
<img src="./关于页面矩形与画布矩形的讨论.assets/用户视界.png" alt="用户视界" style="zoom:67%;" />
...
<img src="./关于页面矩形与画布矩形的讨论.assets/需要渲染的区域.png" alt="需要渲染的区域" style="zoom:80%;" />
...
>
> - [x] 将这部分计算量加入 `ViewPlugin` ？即，再提供一个接口函数，传入鼠标点击的坐标（画布坐标），输出点在哪个页面的某个位置；传入鼠标拖动的矩形（画布坐标），返回像 `Tab.visible_page_positions` 一样的输出矩形列表。
